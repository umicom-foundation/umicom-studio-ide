...
GPtrArray *umi_buildsys_build_argv(const UmiBuildSys *bs){ return split_argv_safe(bs?bs->build_cmd:NULL); }
GPtrArray *umi_buildsys_run_argv  (const UmiBuildSys *bs){ return split_argv_safe(bs?bs->run_cmd:NULL);   }
GPtrArray *umi_buildsys_test_argv (const UmiBuildSys *bs){ return split_argv_safe(bs?bs->test_cmd:NULL);  }
...
static GPtrArray* split_argv_safe(const char* cmd){
  if(!cmd) return g_ptr_array_new_with_free_func(g_free);
  int argc=0; char **argv=NULL; GError *e=NULL;
  if(!g_shell_parse_argv(cmd,&argc,&argv,&e)){
    if(e){ g_warning("argv parse: %s", e->message); g_error_free(e); }
    return g_ptr_array_new_with_free_func(g_free);
  }
  GPtrArray *a=g_ptr_array_new_with_free_func(g_free);
  for(int i=0;i<argc;i++) g_ptr_array_add(a, g_strdup(argv[i]));
  g_strfreev(argv); g_ptr_array_add(a,NULL); return a;
}
