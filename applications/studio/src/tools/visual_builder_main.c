...
int main(void)
{
    UmiStudioVisualBuilderCentre *centre=NULL; UmiStudioVisualBuilderSnapshot snapshot; UmiDesignerBuilderSessionV2 *session;
    ...
    if(umi_designer_builder_session_v2_add_component(session,"welcome","label","root")!=UMI_STATUS_OK||umi_studio_visual_builder_preview_refresh(centre)!=UMI_STATUS_OK||umi_studio_visual_builder_source_refresh(centre)!=UMI_STATUS_OK||umi_studio_visual_builder_centre_snapshot(centre,&snapshot)!=UMI_STATUS_OK){umi_studio_visual_builder_centre_destroy(centre);return EXIT_FAILURE;}
    (void)printf("capability=%s components=%zu generated=%zu preview=%s\n",umi_studio_visual_builder_capability_id(),snapshot.builder.document.component_count,umi_studio_visual_builder_source_count(centre),umi_designer_preview_health_v2_text(snapshot.builder.preview_health));
