...
#include <stdio.h>
#include <string.h>
static UmiStatus add_window(UmiUiWorkspaceLayoutV3 *layout,const char *window_id,const char *title,const char *tool_id,const char *group_id,double x,double y,double width,double height,bool closable,int32_t z_order)
{
    ...
    return umi_ui_workspace_layout_v3_add_window(layout,&window);
}
static UmiStatus develop_layout(UmiUiWorkspaceLayoutV3 *layout)
{
    UmiStatus status = umi_ui_workspace_layout_v3_init(layout,"preset-develop","Develop Template");
    if (status == UMI_STATUS_OK) status = umi_ui_workspace_layout_v3_set_locked(layout,false);
    ...
    if (status == UMI_STATUS_OK) status = umi_ui_workspace_layout_v3_set_locked(layout,true);
    return status;
}
static UmiStatus operations_layout(UmiUiWorkspaceLayoutV3 *layout)
{
    UmiStatus status = umi_ui_workspace_layout_v3_init(layout,"preset-operations","Operations Template");
    if (status == UMI_STATUS_OK) status = umi_ui_workspace_layout_v3_set_locked(layout,false);
...
static UmiStatus compare_layout(UmiUiWorkspaceLayoutV3 *layout)
{
    UmiStatus status = umi_ui_workspace_layout_v3_init(layout,"preset-compare","Compare Template");
    if (status == UMI_STATUS_OK) status = umi_ui_workspace_layout_v3_set_locked(layout,false);
...
static UmiStatus register_preset(UmiUiWorkspaceCustomisationV2 *model,const char *preset_id,const char *category,const char *description,const UmiUiWorkspaceLayoutV3 *layout,const char *instance_id,const char *instance_name)
{
    UmiUiLayoutLibraryItemV2 item;
    UmiUiWorkspaceLayoutV3 instance;
    UmiStatus status;
    ...
    item.layout = *layout;
    status = umi_ui_layout_library_v2_add(&model->library,&item);
    if (status != UMI_STATUS_OK) return status;
    status = umi_ui_layout_library_v2_instantiate(&model->library,preset_id,instance_id,instance_name,&instance);
    if (status != UMI_STATUS_OK) return status;
    return umi_ui_workspace_customisation_v2_add_layout(model,&instance);
}
UmiStatus umi_studio_workspace_layouts_v2_seed(UmiStudioProfessionalWorkspaceV2 *workspace)
{
    ...
    UmiUiWorkspaceLayoutV3 compare;
    UmiStatus status;
    ...
    return umi_ui_workspace_customisation_v2_activate(model,"develop");
}
