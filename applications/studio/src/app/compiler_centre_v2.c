...
#include <stdlib.h>
#include <string.h>
struct UmiStudioCompilerCentreV2 { UmiStudioToolchainCentre *toolchains; UmiCompilerCentre *compiler; UmiStudioCompilerSettingsV2 settings; char active_view[UMI_COMPILER_ID_CAPACITY]; bool toolchain_synchronised; bool project_open; uint64_t revision; };
...
{ UmiStudioCompilerCentreV2 *centre; if (out_centre == NULL) return UMI_STATUS_INVALID_ARGUMENT; *out_centre = NULL; centre = (UmiStudioCompilerCentreV2 *)calloc(1U,sizeof(*centre)); if (centre == NULL) return UMI_STATUS_OUT_OF_MEMORY; centre->compiler = (UmiCompilerCentre *)calloc(1U,sizeof(*centre->compiler)); if (centre->compiler == NULL) { free(centre); return UMI_STATUS_OUT_OF_MEMORY; } centre->toolchains = toolchains; umi_compiler_centre_init(centre->compiler); umi_studio_compiler_settings_v2_defaults(&centre->settings); (void)snprintf(centre->active_view,sizeof(centre->active_view),"compiler.overview"); centre->revision = 1U; *out_centre = centre; return UMI_STATUS_OK; }
void umi_studio_compiler_centre_v2_destroy(UmiStudioCompilerCentreV2 *centre) { if (centre == NULL) return; free(centre->compiler); free(centre); }
...
UmiStatus umi_studio_compiler_centre_v2_open_project(UmiStudioCompilerCentreV2 *centre,const UmiCompilerProjectManifest *manifest) { UmiStatus status; if (centre == NULL) return UMI_STATUS_INVALID_ARGUMENT; status = umi_compiler_centre_add_manifest(centre->compiler,manifest); if (status == UMI_STATUS_OK) { centre->project_open = true; centre->revision += 1U; } return status; }
UmiStatus umi_studio_compiler_centre_v2_activate_view(UmiStudioCompilerCentreV2 *centre,const char *view_id) { if (centre == NULL || view_id == NULL) return UMI_STATUS_INVALID_ARGUMENT; if (umi_studio_compiler_views_v2_find(view_id) == NULL) return UMI_STATUS_NOT_FOUND; (void)snprintf(centre->active_view,sizeof(centre->active_view),"%s",view_id); centre->revision += 1U; return UMI_STATUS_OK; }
UmiStatus umi_studio_compiler_centre_v2_plan_unit(UmiStudioCompilerCentreV2 *centre,const char *project_id,const char *unit_id,UmiCompilerPlan *out_plan) { return centre == NULL ? UMI_STATUS_INVALID_ARGUMENT : umi_compiler_centre_plan_unit(centre->compiler,project_id,unit_id,out_plan); }
UmiStatus umi_studio_compiler_centre_v2_snapshot(const UmiStudioCompilerCentreV2 *centre,UmiStudioCompilerCentreSnapshotV2 *out_snapshot) { if (centre == NULL || out_snapshot == NULL) return UMI_STATUS_INVALID_ARGUMENT; (void)memset(out_snapshot,0,sizeof(*out_snapshot)); umi_compiler_centre_snapshot(centre->compiler,&out_snapshot->compiler); out_snapshot->settings = centre->settings; (void)snprintf(out_snapshot->active_view,sizeof(out_snapshot->active_view),"%s",centre->active_view); out_snapshot->toolchain_synchronised = centre->toolchain_synchronised; out_snapshot->project_open = centre->project_open; out_snapshot->revision = centre->revision + out_snapshot->compiler.revision; return UMI_STATUS_OK; }
UmiCompilerCentre *umi_studio_compiler_centre_v2_engine(UmiStudioCompilerCentreV2 *centre) { return centre == NULL ? NULL : centre->compiler; }
