...
#include <stdlib.h>
#include <string.h>
struct UmiStudioVisualBuilderCentre { UmiDesignerBuilderSessionV2 *session; char active_pane[UMI_DECL_ID_CAPACITY]; uint64_t revision; };
const char *umi_studio_visual_builder_capability_id(void){return "umicom.studio.visual-application-builder-v2";}
UmiStatus umi_studio_visual_builder_centre_create(const char *application_id,UmiStudioVisualBuilderCentre **out_centre)
{
    ...
    *out_centre=NULL;
    ...
    status=umi_designer_builder_session_v2_create(application_id,&centre->session);
    if(status==UMI_STATUS_OK)status=umi_decl_copy_text(centre->active_pane,sizeof(centre->active_pane),"canvas");
    if(status!=UMI_STATUS_OK){umi_designer_builder_session_v2_destroy(centre->session);free(centre);return status;}
    centre->revision=1U; *out_centre=centre; return UMI_STATUS_OK;
}
void umi_studio_visual_builder_centre_destroy(UmiStudioVisualBuilderCentre *centre){if(centre==NULL)return;umi_designer_builder_session_v2_destroy(centre->session);free(centre);}
UmiDesignerBuilderSessionV2 *umi_studio_visual_builder_centre_session(UmiStudioVisualBuilderCentre *centre){return centre!=NULL?centre->session:NULL;}
