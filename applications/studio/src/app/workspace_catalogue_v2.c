...
UmiStatus umi_studio_workspace_catalogue_v2_seed(UmiStudioProfessionalWorkspaceV2 *workspace)
{
    ...
    for (index = 0U; index < sizeof(WINDOWS) / sizeof(WINDOWS[0]); ++index) {
        UmiStatus status = umi_ui_window_catalogue_v2_register(&model->windows,&WINDOWS[index]);
