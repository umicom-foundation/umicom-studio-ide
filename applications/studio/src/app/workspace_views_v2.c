...
#include <stdio.h>
#include <string.h>
typedef struct ViewDefinition { UmiStudioWorkspaceViewKindV2 kind; const char *id; const char *title; const char *summary; } ViewDefinition;
...
size_t umi_studio_workspace_view_v2_count(void) { return sizeof(VIEWS) / sizeof(VIEWS[0]); }
UmiStatus umi_studio_workspace_view_v2_resolve(const UmiStudioProfessionalWorkspaceV2 *workspace,const char *view_id,UmiStudioWorkspaceViewV2 *out_view)
{
    ...
    if (umi_studio_professional_workspace_v2_snapshot(workspace,&snapshot) != UMI_STATUS_OK) return UMI_STATUS_INVALID_STATE;
    for (index = 0U; index < umi_studio_workspace_view_v2_count(); ++index) {
