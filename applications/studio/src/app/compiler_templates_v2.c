...
#include <string.h>
static const UmiStudioCompilerTemplateV2 TEMPLATES[] = {
...
size_t umi_studio_compiler_templates_v2_count(void) { return sizeof(TEMPLATES) / sizeof(TEMPLATES[0]); }
const UmiStudioCompilerTemplateV2 *umi_studio_compiler_templates_v2_at(size_t index) { return index < umi_studio_compiler_templates_v2_count() ? &TEMPLATES[index] : NULL; }
const UmiStudioCompilerTemplateV2 *umi_studio_compiler_templates_v2_find(const char *template_id) { size_t index; if (template_id == NULL) return NULL; for (index = 0U; index < umi_studio_compiler_templates_v2_count(); ++index) if (strcmp(TEMPLATES[index].template_id,template_id) == 0) return &TEMPLATES[index]; return NULL; }
