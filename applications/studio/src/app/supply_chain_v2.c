/*-----------------------------------------------------------------------------
 * Umicom Studio IDE
 * File: applications/studio/src/app/supply_chain_v2.c
 *
 * PURPOSE:
...
void umi_studio_supply_chain_v2_init(UmiStudioSupplyChainV2 *supply_chain)
{
    ...
    umi_release_evidence_v2_init(&supply_chain->evidence);
    ...
    UmiStudioSupplyChainV2 *supply_chain)
{
    ...
    for (index = 0U; index < sizeof(checks) / sizeof(checks[0]); ++index) {
        status = umi_release_evidence_v2_set(
            &supply_chain->evidence, checks[index], UMI_EVIDENCE_PASS);
        if (status != UMI_STATUS_OK) return status;
    }
    return umi_release_evidence_v2_set(
    ...
    UmiReleaseChannel channel)
{
    return supply_chain != NULL &&
           umi_release_evidence_v2_ready(&supply_chain->evidence, channel);
}
