/*-----------------------------------------------------------------------------
 * Umicom Studio IDE
 * File: applications/studio/src/app/sdk_centre_v2.c
 *
 * PURPOSE:
...
UmiStatus umi_studio_sdk_centre_v2_refresh(UmiStudioSdkCentreV2 *centre)
{
    ...
    status = umi_sdk_export_plan_v2_validate(&centre->export_plan);
    if (status != UMI_STATUS_OK) {
        centre->ready = 0;
        return status;
    }
    status = umi_sdk_consumer_package_v2_render_cmake(
                                         ...
                                         const char *install_prefix)
{
    ...
    status = umi_sdk_export_plan_v2_init(&centre->export_plan, install_prefix,
                                          "0.9");
    if (status != UMI_STATUS_OK) return status;
    for (index = 0U; index < sizeof(exports) / sizeof(exports[0]); ++index) {
        status = umi_sdk_export_plan_v2_add_target(
            &centre->export_plan, exports[index].component,
            exports[index].target, 1);
        if (status != UMI_STATUS_OK) return status;
    }
    return umi_studio_sdk_centre_v2_refresh(centre);
}
