...
{
    UmiDesignerBuilderSessionV2 *session=umi_studio_visual_builder_centre_session(centre); UmiDesignerTransactionHistoryV2 *transactions;
    if(session==NULL||command==NULL)return UMI_STATUS_INVALID_ARGUMENT;
    transactions=umi_designer_builder_session_v2_transactions(session);
    switch(command->kind){case UMI_STUDIO_VISUAL_BUILDER_ADD:return umi_designer_builder_session_v2_add_component(session,command->node_id,command->component_type,command->parent_id);case UMI_STUDIO_VISUAL_BUILDER_UNDO:return umi_designer_transaction_history_v2_undo_count(transactions)>0U?umi_designer_transaction_history_v2_undo(transactions):umi_designer_history_undo(umi_designer_builder_session_v2_history(session));case UMI_STUDIO_VISUAL_BUILDER_REDO:return umi_designer_transaction_history_v2_redo_count(transactions)>0U?umi_designer_transaction_history_v2_redo(transactions):umi_designer_history_redo(umi_designer_builder_session_v2_history(session));case UMI_STUDIO_VISUAL_BUILDER_COPY:return umi_studio_visual_builder_copy(centre);case UMI_STUDIO_VISUAL_BUILDER_PASTE:return umi_studio_visual_builder_paste(centre,command->parent_id,command->id_prefix);case UMI_STUDIO_VISUAL_BUILDER_REFRESH_PREVIEW:return umi_studio_visual_builder_preview_refresh(centre);case UMI_STUDIO_VISUAL_BUILDER_GENERATE_SOURCE:return umi_studio_visual_builder_source_refresh(centre);default:return UMI_STATUS_INVALID_ARGUMENT;}
}
