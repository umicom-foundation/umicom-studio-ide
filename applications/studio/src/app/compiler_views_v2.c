...
#include <string.h>
static const UmiStudioCompilerViewV2 VIEWS[] = {
...
size_t umi_studio_compiler_views_v2_count(void) { return sizeof(VIEWS) / sizeof(VIEWS[0]); }
const UmiStudioCompilerViewV2 *umi_studio_compiler_views_v2_at(size_t index) { return index < umi_studio_compiler_views_v2_count() ? &VIEWS[index] : NULL; }
const UmiStudioCompilerViewV2 *umi_studio_compiler_views_v2_find(const char *view_id) { size_t index; if (view_id == NULL) return NULL; for (index = 0U; index < umi_studio_compiler_views_v2_count(); ++index) if (strcmp(VIEWS[index].view_id,view_id) == 0) return &VIEWS[index]; return NULL; }
