...
#include <string.h>
static const UmiStudioCompilerCommandV2 COMMANDS[] = {
...
size_t umi_studio_compiler_commands_v2_count(void) { return sizeof(COMMANDS) / sizeof(COMMANDS[0]); }
const UmiStudioCompilerCommandV2 *umi_studio_compiler_commands_v2_at(size_t index) { return index < umi_studio_compiler_commands_v2_count() ? &COMMANDS[index] : NULL; }
const UmiStudioCompilerCommandV2 *umi_studio_compiler_commands_v2_find(const char *command_id) { size_t index; if (command_id == NULL) return NULL; for (index = 0U; index < umi_studio_compiler_commands_v2_count(); ++index) if (strcmp(COMMANDS[index].command_id,command_id) == 0) return &COMMANDS[index]; return NULL; }
