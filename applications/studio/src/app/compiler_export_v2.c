...
UmiStatus umi_studio_compiler_export_database_v2(const UmiCompilerCentre *centre,char *out_json,size_t capacity) { if (centre == NULL) return UMI_STATUS_INVALID_ARGUMENT; return umi_polyglot_compilation_database_export_json(&centre->compilation_database,out_json,capacity); }
